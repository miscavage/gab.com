...
class GabAPIClient {
    ...
    get currentUser() {
        return {
            ...
            createPost: async(accessToken, postOptions) => {
                ...
                //If not stringified, do it now
                if (!Utils.isString(postOptions)) postOptions = JSON.stringify(postOptions);
            ...
            /**
             * @description Creates a media attachment with given image. Requires write-post scope.
             * @function currentUser.createMediaAttachment()
             * @async
             * @param {string} accessToken - User authorization access token
             * @param {buffer} file - File to upload. Might be jpeg, png or gif. Animated gifs are allowed. Might not be more than 4mb.
             * @param {string} fileName - File name
             * @param {string} fileMimeType - File mime type. Use GabAPIClient.MIME_TYPES[*] for valid options
             * @returns {object} - {success, message, code, data}
             */
            createMediaAttachment: async(accessToken, file, fileName, fileMimeType) => {
                const method = 'POST';
                let path = '/media-attachments/images';

                //Check for parameter warnings
                if (!file) Utils._WARN_('Invalid parameter', 'file must be of type: Buffer.');
                if (!fileName || Utils.isStringEmpty(fileName)) Utils._WARN_('Invalid parameter', 'fileName must be of type: String and greater than 0 characters.');
                if (!fileMimeType || Utils.isStringEmpty(fileMimeType)) Utils._WARN_('Invalid parameter', 'fileMimeType must be of type: String and greater than 0 characters.');

                //Create formData with file
                let formData = { file };

                //Build request options
                let options = this._buildRequestOptions(method, path, accessToken, formData, true);

                //Build binary data
                let binaryData = this._createBinaryPostData(file, fileName, fileMimeType);
                //Set new headers
                options.headers['Content-Type'] = binaryData.headers['Content-Type'];
                options.headers['Content-Length'] = binaryData.headers['Content-Length'];

                //Return request
                return this._request(options, binaryData.postData);
            },

            /**
             * @description Creates a post with a file attachment. Requires write-post scope.
             * @function currentUser.createPostWithAttachment()
             * @async
             * @param {string} accessToken - User authorization access token
             * @param {object[]} fileOptions - [{file: Buffer, fileName: String, fileMimeType: String}, ...] - Max of 4 files
             * @param {object} postOptions - Post options, see: https://developers.gab.com/#949155f0-821e-3228-49ea-4b15f35422a3 for more details
             * @returns {object} - {success, message, code, data}
             */
            createPostWithAttachment: async(accessToken, fileOptions, postOptions) => {
                //If any fileOptions, upload all media in parallel
                if (fileOptions && Utils.isArray(fileOptions)) {
                    if (fileOptions.length > 4) Utils._WARN_('Invalid parameter', 'fileOptions must be of type: Array and not exceed a length of 4.');

                    //Map functions to createMediaAttachments
                    let funcs = fileOptions.map(opts => this.currentUser.createMediaAttachment(accessToken, opts['file'], opts['fileName'], opts['fileMimeType']));

                    //Get all data
                    let mediaAttachmentResponses = await Promise.all(funcs);

                    //Now we only need to get the mediaIds (options: media_attachments)
                    let mediaIds = [];
                    //Cycle now
                    for (let index = 0; index < mediaAttachmentResponses.length; index++) {
                        let res = mediaAttachmentResponses[index];

                        //Check if res.data exists
                        if (!Utils.isObject(res['data'])) {
                            Utils._WARN_('Invalid parameter', 'Error retrieving data from media upload. response.data does not exist.');
                        }
                        else {
                            //Check if res.data.id exists
                            if (!Utils.isString(res.data['id'])) {
                                Utils._WARN_('Invalid parameter', 'Error retrieving data from media upload. response.data.id does not exist.');
                            }
                            else {
                    ...
                    //Append to postOptions now
                    postOptions['media_attachments'] = mediaIds;
                }

                //Return create post now
                return this.currentUser.createPost(accessToken, postOptions);
            },
        }
    }
    ...
    /**
     * @description Create binary post data for uploading images and sending via POST by HTTPS request. More info: https://github.com/coolaj86/node-examples-js/blob/master/http-and-html5/http-upload.js
     * @function _createBinaryPostData
     * @param {buffer} file - File to upload. Might be jpeg, png or gif. Animated gifs are allowed. Might not be more than 4mb.
     * @param {string} fileName - File name
     * @param {string} fileMimeType - File mime type. Use GabAPIClient.MIME_TYPES[*] for valid options
     * @returns {Object} {postData, headers}
     */
    _createBinaryPostData(fileData, fileName, fileMimeType) {
        let crlf = "\r\n";
        ...
        let returnHeaders = {};
        returnHeaders['Content-Type'] = 'multipart/form-data; boundary=' + boundary;
        returnHeaders['Content-Length'] = multipartBody.length;
